...
const SideBar = (props: props) => {
  const { user, theme, settheme, setchats, chats, navigate } = AppuseContext()
  const isAbove=useMedia("(min-width:600px)")
  if(isAbove){
      ...
      <div className=' flex-1 overflow-y-scroll mt-3  text-sm space-y-3'>
        {
          chats?.filter((ct: any) => ct.messages[0] ? ct.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : ct.name.toLowerCase().includes(search.toLowerCase())).map((chat: any) => (
            <div key={chat._id} className='p-2 px-4 mx-1  dark:bg-[#57317c]/10 border border-gray-300 hover:scale-101 dark:border-[#4e90faff] rounded-2xl cursor-pointer flex justify-between group '>
              <div>
              ...
              <img src={assets.bin_icon} alt="bin Icon" className='hidden  group-hover:block w-4 cursor-pointer not-dark:invert ' />
            </div>
          ))
        }
      </div>
